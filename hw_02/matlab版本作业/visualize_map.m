...
%         scatter(map(obs_cnt, 1)-0.5,map(obs_cnt, 2)-0.5,250,155,'filled');
        scatter(map(obs_cnt, 1)-0.5,map(obs_cnt, 2)-0.5,250,'filled','k');
        hold on;
        grid on;
        %grid minor;
        axis equal;        
    ...
    % start point
    scatter(map(1, 1)-0.5, map(1, 2)-0.5,150,'b','h');
    hold on;
    % target point
	scatter(map(size(map, 1), 1)-0.5, map(size(map, 1), 2)-0.5, 150,'r','h');
    hold on;
    %optimal path
    for path_cnt = 2:size(path,1)-1
        scatter(path(path_cnt,1)-0.5,path(path_cnt,2)-0.5,250,'filled','c');
        hold on;
    end
